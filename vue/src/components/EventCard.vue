...
<template>
  <div class="uw-card" @click="$emit('open', event)">
    ...
    <div class="uw-card__content">
      ...
      <p v-if="event.subtitle" class="uw-card__subtitle">{{ event.subtitle }}</p>
      <p class="uw-card__meta">
      ...
      <p v-if="event.summary" class="uw-card__summary">{{ event.summary.length > 300 ? event.summary.substring(0, 300) + '…' : event.summary }}</p>
    </div>
  </div>
</template>
